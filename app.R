...
quartile.w_m<- quantile(maize_outliers$Wholesale, probs=c(.02, .98), na.rm = FALSE)
quartile1.w_w<-quantile(wheat_outliers$Wholesale, probs=c(.02, .98), na.rm = FALSE)

quartile.r_m<- quantile(maize_outliers$Retail, probs=c(.02, .98), na.rm = FALSE)
quartile1.r_w<-quantile(wheat_outliers$Retail, probs=c(.02, .98), na.rm = FALSE)
...
ui <- fluidPage(
    ...
    titlePanel("Price changes of grains as a result of Russia-Ukraine War"),
       HTML("<p> The dashboard represents retail & wholesale prices of grains in Kenya. Available data has been mined from the
       <a href='http://amis.co.ke/site/market'>Ministry of Agriculture</a> website. The data is updated regularly across several counties. 
       I have done a trend analysis as well as before-after analysis on price changes of maize and wheat in Kenya as a result of Russia-Ukraine War. The reference
            date is 24th February 2022 when war began. 
            Notably, no causal analysis has been done on whether the changes are attributed to the war: That said,
    ...
    mainPanel(
        tabsetPanel(
            tabPanel("Mined data",DT::DTOutput("data.t")),
                     tabPanel("Price trend",plotOutput("plot1")),
...
server <- function(input, output) {
    ...
    output$plot1<-renderPlot({
        options(digits=2)
        cols <- c("#5CB85C", "#EEA236")
        plot.d=combined.d() %>%
            melt(id.vars =c("Month","Year")) %>%
            mutate(Year_month=paste(Month,Year,sep="-")) %>%
            mutate(Year_month=as.yearmon(Year_month, "%b-%y")) %>%
            as.data.frame()
    
        ggplot(plot.d, aes(x = Year_month, y = value, color = variable)) +
            geom_line() +
            xlab("Period") +
            ylab("Price per KG (KSh)") +
